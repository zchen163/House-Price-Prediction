...
for line in data:
    ...
    try:
        ...
        print('pop is', pop_total, pop_dens, age[0])
        ...
        other = 100 - float(white[0]) - float(black[0]) - float(asian[0]) - float(hispanic[0])
        print(white[0], black[0], asian[0], hispanic[0], other)

        result = [str(zip1), pop_total, pop_dens, age[0], white, black, asian, hispanic, other]
        print(result)
        table.append(result)
    except:
        result = [str(zip1), '0', '0', '0', '0', '0', '0', '0', '0']
        table.append(result)


header = ['zipcode', 'population', 'population density (per square miles)', 'average age', 'race white percentage', 'race black percentage', 'race asian percentage', 'race hispanic percentage', 'race other percentage']
# write into newfile:
